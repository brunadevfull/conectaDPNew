    ...
    $.ajax({
        ...
        success: function(response) {
            try {
              ...
              // Se for um array de arrays, desaninha os dados
if (Array.isArray(jsonResponse) && jsonResponse.length > 0 && Array.isArray(jsonResponse[0])) {
    jsonResponse = jsonResponse[0]; // Pegamos apenas o primeiro nível interno
}
    ...
    if (option === 'CNPJ') {
        ...
        return data.map(function (item) {
            return {
                ...
                'situacaoCadastralCodigo': item.situacaoCadastral?.codigo || '',
                'situacaoCadastralData': item.situacaoCadastral?.data || '',
                'situacaoCadastralMotivo': item.situacaoCadastral?.motivo || '',
                'naturezaJuridicaDescricao': item.naturezaJuridica?.descricao || '',
                'dataAbertura': item.dataAbertura || '',
                'enderecoTipoLogradouro': item.endereco?.tipoLogradouro || '',
                'enderecoLogradouro': item.endereco?.logradouro || '',
                'enderecoNumero': item.endereco?.numero || '',
                'enderecoComplemento': item.endereco?.complemento || '',
                'enderecoCep': item.endereco?.cep || '',
                'enderecoBairro': item.endereco?.bairro || '',
                'enderecoMunicipioCodigo': item.endereco?.municipio?.codigo || '',
                'enderecoMunicipioDescricao': item.endereco?.municipio?.descricao || '',
                'enderecoUf': item.endereco?.uf || '',
                'enderecoPaisCodigo': item.endereco?.pais?.codigo || '',
                'enderecoPaisDescricao': item.endereco?.pais?.descricao || '',
                'municipioJurisdicaoCodigo': item.municipioJurisdicao?.codigo || '',
                'municipioJurisdicaoDescricao': item.municipioJurisdicao?.descricao || '',
                'telefones': item.telefones ? item.telefones.map(tel => `(${tel.ddd}) ${tel.numero}`).join(', ') : '',
        ...
        // Itera sobre cada CPF no retorno da API
        data.forEach(cpfData => {
            if (!cpfData.relacoesTrabalhistas || !Array.isArray(cpfData.relacoesTrabalhistas)) {
                console.warn('Nenhuma relação trabalhista encontrada para este CPF:', cpfData);
                return;
            }

            // Itera sobre cada vínculo dentro do CPF
            cpfData.relacoesTrabalhistas.forEach(vinculo => {
                formattedData.push({
                    'Tipo de Inscrição': vinculo.empregador?.tipoInscricao || '',
                    'Número de Inscrição': vinculo.empregador?.numeroInscricao || '',
                    'Data de Admissão': vinculo.dataAdmissao || '',
                    'Data de Encerramento': vinculo.dataEncerramento || 'Ativo',
                    'CBO Código': vinculo.cbo?.codigo || '',
                    'CBO Descrição': vinculo.cbo?.descricao || '',
                    'Última Remuneração': vinculo.ultimaRemuneracao?.competencia || '',
                    'Valor Última Remuneração': vinculo.ultimaRemuneracao?.valor || 'Não informado',
                    'Pendências': vinculo.pendencias && vinculo.pendencias.length > 0
                        ? vinculo.pendencias.map(p => `${p.codigo}: ${p.descricao}`).join(', ')
                        : 'Sem pendências'
                });
            });
        });

        console.log('Dados formatados para o grid (CNIS):', formattedData);
...
function getColumnsForOption(option) {
    if (option === 'CPF') {
        ...
        return [
            { text: 'Tipo de Inscrição', datafield: 'Tipo de Inscrição', width: '15%' },
            { text: 'Número de Inscrição', datafield: 'Número de Inscrição', width: '20%' },
            { text: 'Data de Admissão', datafield: 'Data de Admissão', width: '15%' },
            { text: 'Data de Encerramento', datafield: 'Data de Encerramento', width: '15%' },
            { text: 'CBO Código', datafield: 'CBO Código', width: '10%' },
            { text: 'CBO Descrição', datafield: 'CBO Descrição', width: '25%' },
            { text: 'Última Remuneração', datafield: 'Última Remuneração', width: '10%' },
            { text: 'Valor Última Remuneração', datafield: 'Valor Última Remuneração', width: '15%' },
            { text: 'Pendências', datafield: 'Pendências', width: '20%' }
        ];
    }
    return [];
}
