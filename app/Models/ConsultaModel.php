...
public function obterTokenOAuth2($tokenUrl, $clientId, $clientSecret, $opcao) {
 ...
 public function registrarConsultaUsuario($username, $documento, $tipoDocumento) {
    try {
        $query = "INSERT INTO consultas_usuario (username, documento, data_consulta, tipo_documento) 
                  VALUES (:username, :documento, NOW(), :tipoDocumento)";
        ...
        error_log("Erro ao registrar consulta do usuário: " . $e->getMessage());
        return false;
    }
}
        ...
        try {
            ...
            foreach ($worksheet->getRowIterator() as $row) {
                ...
                if (!empty($cellValue)) {
                 ...
                 $cpfSemPontos = preg_replace('/\D/', '', trim($cellValue)); // Remove tudo que não for número
$cpfCorrigido = str_pad($cpfSemPontos, 11, '0', STR_PAD_LEFT); // Garante 11 dígitos

// Verifica se tem apóstrofo (')
if (strpos($cpfCorrigido, "'") !== false) {
    $cpfCorrigido = str_replace("'", "", $cpfCorrigido);
}
...
return $cpfCorrigido;
    ...
    public function usuarioTemPermissao($username, $recurso) {
        try {
            // 1. Busca o perfil do usuário
            $stmt = $this->conexao->prepare("SELECT id, perfil FROM usuario WHERE username = :username");
            $stmt->bindParam(':username', $username);
            $stmt->execute();
            $usuario = $stmt->fetch(\PDO::FETCH_ASSOC);
            ...
            // 2. Se for admin, tem permissão total
            if ($usuario['perfil'] === 'admin') {
                return true;
            }
    
            // 3. Se for comum, verifica permissões explícitas
            $stmt = $this->conexao->prepare("
                SELECT permitido FROM permissoes_api
                WHERE id_usuario = :id_usuario AND recurso = :recurso
            ");
            ...
            return $permitido === true || $permitido === 't';
        } catch (\PDOException $e) {
            error_log("Erro ao verificar permissão do usuário: " . $e->getMessage());
            return false;
        }
    }
 
    public function obterRecursosPermitidos($username) {
        try {
            // Primeiro, verifica o perfil
            $queryPerfil = "SELECT perfil FROM usuario WHERE username = :username";
            $stmtPerfil = $this->conexao->prepare($queryPerfil);
            $stmtPerfil->bindParam(':username', $username);
            $stmtPerfil->execute();
            $perfil = $stmtPerfil->fetchColumn();
    
            // Se for admin, retorna todas as opções
            if ($perfil === 'admin') {
                return ['CPF', 'CNPJ', 'CNIS'];
            }
    
            // Caso contrário, busca na tabela permissoes_api
            $queryPermissoes = "
                SELECT recurso FROM permissoes_api 
                JOIN usuario ON permissoes_api.id_usuario = usuario.id 
                WHERE username = :username AND permitido = true
            ";
            $stmt = $this->conexao->prepare($queryPermissoes);
            $stmt->bindParam(':username', $username);
            $stmt->execute();
